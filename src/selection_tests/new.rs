...
#[test] fn no_extension_bar_semantics(){
    ...
    assert_eq!(Direction::Forward, selection.direction(&text, CursorSemantics::Bar));
}
...
#[test] fn extended_backward_bar_semantics(){
    ...
    assert_eq!(0, selection.head());
    assert_eq!(Direction::Backward, selection.direction(&text, CursorSemantics::Bar));
}
...
#[test] fn no_extension_forward_cursor_block_semantics(){
    ...
    assert_eq!(0, selection.anchor());
    assert_eq!(1, selection.head());
    assert_eq!(Direction::Forward, selection.direction(&text, CursorSemantics::Block));
}
#[test] fn no_extension_backward_cursor_block_semantics(){
    ...
    assert_eq!(1, selection.anchor());
    assert_eq!(0, selection.head());
    assert_eq!(Direction::Backward, selection.direction(&text, CursorSemantics::Block));
}
