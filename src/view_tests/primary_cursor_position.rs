...
fn primary_cursor_position(){
    ...
    let selections = Selections::new(vec![Selection::new(Range::new(0, 3), Direction::Forward)], 0, &text, CursorSemantics::Bar);
    assert_eq!(Some(Position::new(3, 0)), view.primary_cursor_position(&text, &selections, CursorSemantics::Bar));

    let text = Rope::from("idk\nsome\nshit\n");
    let view = View::new(0, 0, 5, 5);
    //let selections = Selections::new(vec![Selection::new(0, 3)], 0, &text);
    let selections = Selections::new(vec![Selection::new(Range::new(0, 3), Direction::Forward)], 0, &text, CursorSemantics::Block);
...
fn primary_cursor_position_with_cursor_outside_view(){
    ...
    let selections = Selections::new(vec![Selection::new(Range::new(9, 13), Direction::Forward)], 0, &text, CursorSemantics::Bar);
    assert_eq!(None, view.primary_cursor_position(&text, &selections, CursorSemantics::Bar));

    let text = Rope::from("idk\nsome\nshit\n");
    let view = View::new(0, 0, 5, 1);
    //let selections = Selections::new(vec![Selection::new(9, 13)], 0, &text);
    let selections = Selections::new(vec![Selection::new(Range::new(9, 13), Direction::Forward)], 0, &text, CursorSemantics::Block);
