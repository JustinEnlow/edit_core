...
fn selections_impl(selections: &Selections) -> Result<Selections, SelectionsError>{
    ...
    if selections.primary_selection_index.saturating_add(1) < selections.count(){
        ...
        Ok(Selections{selections: selections.selections.clone(), primary_selection_index: 0})
    }
}

#[cfg(test)]
mod tests{
    use crate::utilities::increment_primary_selection;
    use crate::{
        document::Document,
    ...
    //    let result = increment_primary_selection::document_impl(&mut doc);
    ...
    //        .with_text(text.clone())
    //        .with_selections(Selections::new(selections, primary, &text, semantics));
    //    assert!(increment_primary_selection::document_impl(&mut doc).is_err());
    //}
    ...
    #[test] fn wraps_if_primary_is_last(){
        //test(
        //    "idk\nsome\nshit\n", 
        //    vec![
        //        Selection::new(Range::new(0, 1), Direction::Forward),
