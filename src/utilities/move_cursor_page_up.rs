...
pub fn selection_impl(selection: &Selection, text: &Rope, client_view: &View, semantics: CursorSemantics) -> Result<Selection, SelectionError>{
    ...
    selection.move_vertically(client_view.height().saturating_sub(1), text, Movement::Move, Direction::Backward, semantics)
}

#[cfg(test)]
mod tests{
    use crate::utilities::move_cursor_page_up;
    use crate::{
        document::Document,
    ...
    fn test(semantics: CursorSemantics, text: &str, selections: Vec<Selection>, primary: usize, expected_selections: Vec<Selection>, expected_primary: usize){
        ...
        let result = move_cursor_page_up::document_impl(&mut doc, semantics);
    ...
    fn test_error(semantics: CursorSemantics, text: &str, selections: Vec<Selection>, primary: usize){
        ...
        let mut doc = Document::new(semantics)
            .with_text(text.clone())
            .with_selections(Selections::new(selections, primary, &text, semantics));
        assert!(move_cursor_page_up::document_impl(&mut doc, semantics).is_err());
        assert!(!doc.is_modified());
    }
