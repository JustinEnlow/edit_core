...
pub fn selection_impl(selection: &Selection, text: &Rope, semantics: CursorSemantics) -> Result<Selection, SelectionError>{
    ...
    if selection.cursor(text, semantics) == line_start{return Err(SelectionError::ResultsInSameState);}
    selection.put_cursor(line_start, text, Movement::Extend, semantics, true)
}
...
mod tests{
    ...
    //    let result = extend_selection_line_start::document_impl(&mut doc, semantics);
    ...
    //        .with_text(text.clone())
    //        .with_selections(Selections::new(selections, primary, &text, semantics));
    //    assert!(extend_selection_line_start::document_impl(&mut doc, semantics).is_err());
    //}
